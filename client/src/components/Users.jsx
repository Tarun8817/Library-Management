...
const Users = () => {
  ...
  const formatDate = (timeStamp) => {
    ...
    :${String(date.getMinutes()).padStart(2, "0")}:${String(date.getSeconds()
    ).padStart(2, "0")}`;
    ...
    <>
      <main className="relative flex-1 p-6 pt-28">
        ...
        <header className="flex flex-col gap-3 md:flex-row md:justify-between md:items-center">
          <h2 className="text-xl font-medium md:text-2xl md:font-sm">Registered Users</h2>
        </header>
                ...
                <tbody>
                  {users?.filter(u => u.role === "User").map((user, index) => (
                    <tr key={user._id} className={(index + 1) % 2 === 0 ? "bg-gray-50" : ""}>
                      ...
                      <td className="px-4 py-2">{user.booksBorrowed?.length || 0}</td>
